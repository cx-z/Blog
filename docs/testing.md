...
4. 执行 HTTP 集成测试（`test/test-e2e.py`，覆盖注册/登录/文章权限/注销账号等关键路径）
...
- HTTP 集成测试会使用管理员账号覆盖“管理员可见全量文章/管理员软删”等场景；如未提供管理员账号，相关断言会被跳过
- HTTP 集成测试会在临时数据库内执行“注销账号”，不会影响仓库中的快照数据库
